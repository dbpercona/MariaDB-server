	...
	for(ulint i=0; i < n_items;i++) {
		...
		if (sync_arr_get_item(i, &cell)) {
			...
			if (type == SYNC_MUTEX) {
				...
				|| type == RW_LOCK_WAIT_EX
				|| type == RW_LOCK_SHARED) {
				rw_lock_t* rwlock = (rw_lock_t*)cell->old_wait_rw_lock;
				...
				if (rwlock) {
					...
					OK(field_store_string(fields[SYS_SEMAPHORE_WAITS_CREATED_FILE], innobase_basename(rwlock->cfile_name)));
					OK(field_store_ulint(fields[SYS_SEMAPHORE_WAITS_CREATED_LINE], rwlock->cline));
